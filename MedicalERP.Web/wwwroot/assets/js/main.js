  ...
  if (window.templateCustomizer) {
    ...
    if (window.Helpers.isLightStyle()) {
      ...
      if (styleSwitcherToggleEl) {
        ...
        switchImage('dark');
        switchDevExtremeTheme('dark');
    }
  } else {
    // Removed style switcher element if not using template customizer
    styleSwitcherToggleEl.parentElement.remove();
  }
    function switchDevExtremeTheme(mode) {
       debugger
        const existingLink = document.querySelector('link[href*="cdn3.devexpress.com/jslib"]');

        // Remove the existing theme if found
        if (existingLink) {
            existingLink.remove();
        }

        // Define your theme URLs
        const themes = {
            light: "https://cdn3.devexpress.com/jslib/22.1.4/css/dx.material.blue.light.compact.css",
            dark: "https://cdn3.devexpress.com/jslib/22.1.4/css/dx.material.blue.dark.compact.css" // Change this to the correct dark theme URL if different
        };
